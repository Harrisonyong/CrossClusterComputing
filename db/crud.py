...
def create_partition(db: Session, partition: schema.PartitionCreate):
    ...
    db_partition = models.PartitionStatus(cluster_name=partition.cluster_name, partition_name=partition.partition_name,
                                          avail=partition.avail, nodes=partition.nodes,
                                          nodes_avail=partition.nodes_avail, state=partition.state)
    db.add(db_partition)
    db.commit()
...
def update_partition(db: Session, cluster_name: str, partition_name: str, nodes: int, nodes_avail: int, avail: str,
                     state: str):
    """
    更新集群分区信息
    :param db: 数据库会话
    ...
    :return 更新的分区信息
    """
    db_partition = db.query(models.PartitionStatus).filter(models.PartitionStatus.cluster_name == cluster_name,
                                                           models.PartitionStatus.partition_name == partition_name).update(
        {"nodes": nodes, "nodes_avail": nodes_avail, "avail": avail, "state": state})
    db.commit()
    return db_partition
